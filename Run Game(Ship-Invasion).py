...
def run_game():
    ...
    play_button = Button(
        ai_settings, screen, "|   PRESS  Enter  Key  To  START   |   USE  Spacebar  Key  To  SHOOT   |")
    game_over = Game_Over(ai_settings, screen,
                          "GAME - OVER   |   Ship - Invaded")
# \n use left-right arrowkeys to move \n use spacebar key to shoot.
    # Load the high score.
    gf.load_score(stats)
    sb.prep_high_score()
    sb.show_score()
    ...
    while True:
        ...
        gf.check_events(ai_settings, screen, stats, play_button,
                        ship, aliens, bullets, sb, enemy_bullets)
        if stats.game_active:
            ship.update()
            gf.update_bullets(ai_settings, screen, stats, sb,
                              ship, aliens, bullets)
            gf.update_enemy_bullets(
                ai_settings, screen, stats, sb, ship, aliens, enemy_bullets)
            gf.update_aliens(ai_settings, stats, sb, screen,
                             ship, aliens, bullets, enemy_bullets, game_over)
